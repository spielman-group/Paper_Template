...
4) **Do not rewrite large sections of prose unprompted.**
...
* **`coauthor`**: For real-time writing assistance, paragraph-by-paragraph, using LaTeX macros. Requires `AGENTS/RESULTS.md`.
...
*(Note: Detailed instructions and templates for these skills are located in `.agents/skills/`. State files and artifacts generated by these skills are saved in the `AGENTS/` directory.)*